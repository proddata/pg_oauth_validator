...
Distribution packagers pin a source URL together with a digest, so every
release must expose an immutable source tarball whose checksum is published by
this project rather than computed by the consumer.

- The tarball is the tag's generated source archive:
  `https://github.com/proddata/pg_oauth_validator/archive/refs/tags/v<version>.tar.gz`.
  Because the tag is annotated, signed, and never moved or reused, the tree
  behind that URL is immutable.
- After pushing the tag, run `make release-source-checksums RELEASE_TAG=v<version>`
  and publish the output verbatim in the release notes. It records the tag, the
  commit, the source URL, and both the SHA-256 and the SHA-512 of the tarball.
  Publishing SHA-512 as well as SHA-256 is deliberate: some packaging systems
  require it, and a consumer must never have to compute the digest it is
  supposed to be verifying against.
- That target refuses to run for a lightweight tag or a tag whose signature
  does not verify, and it extracts the published tarball and compares it
  against a local `git archive` of the same tag. Checksums are published only
  for a tarball proven to contain exactly the signed tag's tree.
- Known limitation: this tarball is generated by the forge, not built by this
  project, and its bytes have historically changed when the forge changed its
  archive implementation, even though the tree did not. If a downstream digest
  ever stops matching, treat it as a forge change rather than a compromise,
  re-run the target above to re-establish the reviewed digest, and publish a
  project-built release asset if a consumer needs a tarball this project
  produces itself. Do not move the tag.
- Version floors, link modes, and the fatal-warning opt-out that packagers rely
  on are specified in [`dependencies.md`](dependencies.md). They are part of the
  published interface: changing a floor or removing a switch is a compatibility
  change subject to the version policy above.
